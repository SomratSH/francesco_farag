      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 15),
            const FormLabel(label: 'Amount *', icon: Icons.attach_money),
            const CustomTextField(hintText: 'Enter amount'),
            
            const SizedBox(height: 15),
            const FormLabel(label: 'Due Date *', icon: Icons.calendar_today_outlined),
            ...
            GridView.count(
              ...
              mainAxisSpacing: 10,
              children: const [
                FineTypeTile(icon: Icons.local_hospital, label: 'Speeding Violation', color: Colors.red),
                FineTypeTile(icon: Icons.traffic, label: 'Traffic Light Violation', color: Colors.green),
                FineTypeTile(icon: Icons.local_parking, label: 'Parking Violation', color: Colors.blue),
                FineTypeTile(icon: Icons.warning_amber_rounded, label: 'ZTL Violation', color: Colors.orange),
                FineTypeTile(icon: Icons.directions_car, label: 'Toll Violation', color: Colors.green),
                FineTypeTile(icon: Icons.build_outlined, label: 'Vehicle Damage', color: Colors.grey),
                FineTypeTile(icon: Icons.assignment_outlined, label: 'Other', color: Colors.brown),
              ],
            ),

            // --- Upload PDF Section ---
            const SizedBox(height: 25),
            const SectionHeader(icon: Icons.picture_as_pdf_outlined, title: 'Upload Fine PDF'),
            Container(
              ...
              child: Column(
                children: const [
                  ...
                  SizedBox(height: 8),
                  Text('Upload Fine Document', style: TextStyle(fontWeight: FontWeight.w500)),
                  Text('PDF format recommended', style: TextStyle(color: Colors.grey, fontSize: 12)),
                ],
              ),
            ),
            ...
            // --- Additional Note ---
            const SizedBox(height: 20),
            const SectionHeader(icon: Icons.note_add_outlined, title: 'Additional Note'),
            const TextField(
              maxLines: 4,
                ...
                Expanded(
                  child: Container(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text('Send Fine to Client', 
...
class FineTypeTile extends StatelessWidget {
  ...
  const FineTypeTile({super.key, required this.icon, required this.label, required this.color});
